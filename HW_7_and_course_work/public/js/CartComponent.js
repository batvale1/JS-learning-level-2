Vue.component('cart', {
    ...
    methods: {
        ...
        remove(product){
            this.$parent.postJson(`api/cart/delete/${product.id_product}`, product)
                .then(data => {
                    if(data.result){
    ...
    template: `<div v-if="!detail_cart" class="cart_block__product" >
            ...
            <div v-else class="shopping_cart__product_info container">
                ...
                <form class="shopping_cart__element" action="#" @change.prevent.stop="$emit('addProduct', cartItem, $event.target.value)">
                    <input type="text" :value="cartItem.quantity" onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57">
                </form>
